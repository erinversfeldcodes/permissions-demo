...
export const createDepthLimitRule = (
  maxDepth: number = defaultConfig.maxDepth,
): ValidationRule => {
  return (context) => {
    let currentDepth = 0;
 ...
 */
export const createFieldLimitsRule = (
...
export const createCustomValidationRules = (
  config?: Partial<ValidationRuleConfig>,
): ValidationRule[] => {
...
export const logQueryMetrics = (
  ...
  console.log(
    `GraphQL Query: ${operationName || "anonymous"} | Cost: ${cost} | Depth: ${depth}`,
  );
};
