  ...
  Query: {
    ...
    user: async (
      ...
      return user
        ? enrichUser({
            ...user,
            permissions: user.permissions.map(enrichPermission),
          })
        : null;
    },
    ...
    users: async (
      ...
      if (first && first > 50) {
        throw new Error(
          "Maximum limit is 50 items for users query. Please use pagination for larger datasets.",
        );
      }
    ...
    accessibleUsers: async (
      ...
      if (first && first > 100) {
        throw new Error(
          "Maximum limit is 100 items. Please use pagination for larger datasets.",
        );
      }
    ...
    organizationNode: async (
      ...
      const [parent, children, users] = await Promise.all([
        node.parentId
          ? context.dataloaders.organizationNodeById.load(node.parentId)
          : null,
    ...
    updateUser: async (
      ...
      try {
        ...
        if (input.organizationNodeId)
          updateData.organizationNodeId = input.organizationNodeId;
      ...
      try {
        ...
        const appError =
          error instanceof Error
            ? error
            : new Error("Failed to create organization node");
        return {
          success: false,
          errors: [
            {
    ...
    updateOrganizationNode: async (
      ...
      try {
        ...
        if (
          ...
          input.parentId !== existingNode.parentId
        ) {
          let newLevel = 0;
          if (input.parentId) {
            const parent = await db.organizationNode.findUnique({
        ...
        const appError =
          error instanceof Error
            ? error
            : new Error("Failed to update organization node");
        return {
          success: false,
          errors: [
            {
    ...
    deleteOrganizationNode: async (
      ...
      try {
        ...
        const appError =
          error instanceof Error
            ? error
            : new Error("Failed to delete organization node");
        return {
          success: false,
          errors: [
            {
