...
describe('User Entity', () => {
  ...
  describe('setPassword', () => {
    test('should update password hash', async () => {
      ...
      // Verify the password was updated by testing authentication
      await expect(testUser.verifyPassword(newPassword)).resolves.toBe(true)
      // Old password should no longer work
      await expect(testUser.verifyPassword('original_password')).resolves.toBe(false)
    })

    test('should validate password strength', async () => {
      await expect(testUser.setPassword('weak')).rejects.toThrow(ValidationError)
    })

    test('should require minimum password length', async () => {
      await expect(testUser.setPassword('12345')).rejects.toThrow(ValidationError)
    })
  ...
  describe('recordLogin', () => {
    test('should record current timestamp on login', () => {
      ...
      expect(testUser.lastLoginAt!.getTime()).toBeGreaterThanOrEqual(beforeLogin.getTime())
      expect(testUser.lastLoginAt!.getTime()).toBeLessThanOrEqual(afterLogin.getTime())
    })
    ...
    test('should update last login when called multiple times', () => {
      ...
      setTimeout(() => {
        testUser.recordLogin()
        expect(testUser.lastLoginAt!.getTime()).toBeGreaterThan(firstLogin!.getTime())
      }, 10)
    })
  })
  ...
  describe('Business Rules', () => {
    ...
    test('should maintain audit trail through timestamps', () => {
      ...
      expect(testUser.createdAt.getTime()).toBeLessThanOrEqual(testUser.updatedAt.getTime())
    })
  })
  ...
  describe('Error Handling', () => {
    ...
    test('should handle null/undefined values gracefully', () => {
      ...
      expect(() => testUser.changeName(undefined as any)).toThrow(ValidationError)
    })
  })
})